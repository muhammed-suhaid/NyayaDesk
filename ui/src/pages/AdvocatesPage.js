...
import {
  Alert, Box, Button, Card, Dialog, DialogActions, DialogContent, DialogTitle, Stack, TextField, Typography, Table, TableBody, TableCell, TableHead, TableRow, Avatar, IconButton, Chip, alpha, useTheme
} from '@mui/material';
...
export default function AdvocatesPage() {
  ...
  const validate = () => {
    ...
    if (!passwordMinLen(form.password, 6)) next.password = 'Min 6 chars';
    if (!required(form.barCouncilNumber)) next.barCouncilNumber = 'Required';
    setErrors(next);
    ...
    <Box sx={{ maxWidth: 1200, mx: 'auto', py: 1.5 }}>
      <Stack spacing={2}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {isAdmin && <Button variant="contained" size="small" startIcon={<AddIcon />} onClick={() => setOpen(true)} sx={{ fontWeight: 800 }}>Add Staff</Button>}
        </Box>
                ...
                <TableRow key={a.id} hover>
                  ...
                  <TableCell align="right">
                    {isAdmin && <IconButton size="small" color="error" onClick={async () => {
                      if(window.confirm('Remove?')) { await AdvocatesApi.remove(a.id); load(); }
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="xs" fullWidth>
        <DialogTitle sx={{ fontWeight: 900 }}>Add Staff</DialogTitle>
        <DialogContent dividers sx={{ py: 2 }}>
          <Stack spacing={2}>
            ...
            <TextField fullWidth label="Password" size="small" type="password" value={form.password} onChange={e => setForm({...form, password: e.target.value})} error={!!errors.password} />
            <TextField fullWidth label="BCN Number" size="small" value={form.barCouncilNumber} onChange={e => setForm({...form, barCouncilNumber: e.target.value})} error={!!errors.barCouncilNumber} />
          </Stack>
        </DialogContent>
        <DialogActions><Button onClick={() => setOpen(false)}>Cancel</Button><Button variant="contained" onClick={async () => { if(validate()) { await AdvocatesApi.create(form); setOpen(false); load(); } }}>Add</Button></DialogActions>
      </Dialog>
    </Box>
  );
}
