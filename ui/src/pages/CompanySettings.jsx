...
export default function CompanySettings() {
  ...
  const handleSave = async () => {
    setStatus({ type: '', message: '' });
    const next = {};
    if (!required(name)) next.name = 'Name required';
    if (email && !isValidEmail(email)) next.email = 'Invalid email';
    if (!isValidPhoneOptional10Digit(phone)) next.phone = 'Invalid phone';
      ...
      <Card>
        <CardContent>
          <Stack spacing={2}>
            ...
            <Box>
              <Button variant="outlined" onClick={handleOpen}>Edit Info</Button>
            </Box>
          </Stack>
        </CardContent>
      </Card>
